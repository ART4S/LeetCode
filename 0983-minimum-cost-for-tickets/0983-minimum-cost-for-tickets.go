func mincostTickets(days []int, costs []int) int {
	...
	n, m := len(days)-1, days[len(days)-1]
	...
	for i := 1; i <= n; i++ {
		for j := days[i]; j <= m; j++ {
			...
			if j == days[i] {
				buy1 = costs[0] + dp[i-1][days[i-1]]
			}

			if j < days[i]+7 {
				buy7 = costs[1] + dp[i-1][days[i-1]]
			}

			if j < days[i]+30 {
				buy30 = costs[2] + dp[i-1][days[i-1]]
			} else {
				break
			}

			dp[i][j] = Min4(notBuy, buy1, buy7, buy30)
		}
	}
...
func mincostTickets_memo(days []int, costs []int) int {
	n, m := len(days), 365*30
	...
	solve = func(i, pass int) int {
		...
		if memo[i][pass] == -1 {
			...
			if pass > days[i] {
				notBuy = solve(i+1, pass)
			} else {
				pass = days[i]
			}

			buy1 := costs[0] + solve(i+1, pass+1)
			buy7 := costs[1] + solve(i+1, pass+7)
			buy30 := costs[2] + solve(i+1, pass+30)

			memo[i][pass] = Min4(notBuy, buy1, buy7, buy30)
		}
