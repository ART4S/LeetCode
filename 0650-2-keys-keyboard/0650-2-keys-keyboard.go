func minSteps(n int) int {
	...
	solve = func(len, copied int) int {
		if len == n {
			return 0
		}

		if memo[len][copied] == -1 {
			copy, paste := math.MaxInt, math.MaxInt

			if copied != len && len+len <= n {
				copy = solve(len, len)
			}

			if len+copied <= n {
				paste = solve(len+copied, copied)
			}

			if copy == math.MaxInt && paste == math.MaxInt {
				memo[len][copied] = math.MaxInt
			} else {
				memo[len][copied] = 1 + Min(copy, paste)
			}
		}

		return memo[len][copied]
	}
