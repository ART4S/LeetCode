func maxProfit(prices []int, fee int) int {
	pbuy, psell, pskip := -prices[0], 0, 0
	cbuy, csell, cskip := 0, 0, 0

	for i := 1; i < len(prices); i++ {
		cbuy = Max(pbuy, Max(psell, pskip)-prices[i])
		csell = Max(psell, pbuy+prices[i]-fee)
		cskip = Max3(pskip, pbuy, psell)

		pbuy, psell, pskip = cbuy, csell, cskip
	}

	return Max3(cbuy, csell, cskip)
}

func maxProfit_memo(prices []int, fee int) int {
	...
	solve = func(i, op int) int {
		if i == len(prices) {
			return 0
		}

		if memo[i][op] == math.MinInt {
			pskip := solve(i+1, op)
			pbuy := 0
			psell := 0

			if op == 0 {
				pbuy = solve(i+1, 1) - prices[i]
			} else {
				psell = solve(i+1, 0) + prices[i] - fee
			}

			memo[i][op] = Max3(pskip, pbuy, psell)
		}
