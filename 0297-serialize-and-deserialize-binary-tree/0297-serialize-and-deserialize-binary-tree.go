...
func (this *Codec) serialize(root *TreeNode) string {
	...
	for que.Len() > 0 {
		...
		for i := 0; i < levelLen; i++ {
			...
			level[p.index] = strconv.Itoa(p.node.Val)

			if p.node.Left != nil {
				que.Push(&pair{2 * i, p.node.Left})
			}

			if p.node.Right != nil {
				que.Push(&pair{2*i + 1, p.node.Right})
			}
		}
...
func (this *Codec) deserialize(data string) *TreeNode {
	...
	treeArr := strings.Split(data, ",")
	root := &TreeNode{}
	root.Val, _ = strconv.Atoi(treeArr[0])
	prevLevel := []*TreeNode{root}

	for i, curLevelMaxLen := 1, 0; i < len(treeArr); i += curLevelMaxLen {
		curLevelMaxLen = 2 * len(prevLevel)
		curLevel := make([]*TreeNode, 0)

		for j := 0; j < curLevelMaxLen; j++ {
			strval := treeArr[i+j]
			if strval != "" {
				node := &TreeNode{}
				node.Val, _ = strconv.Atoi(strval)

				if j%2 == 0 {
					prevLevel[j/2].Left = node
				} else {
					prevLevel[j/2].Right = node
				}
