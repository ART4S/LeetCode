func isInterleave(s1 string, s2 string, t string) bool {
...
func isInterleave_memo(s1 string, s2 string, t string) bool {
	memo := make([][][]int, len(s1)+1)
	for i := range memo {
		memo[i] = make([][]int, len(s2)+1)
		for j := range memo[i] {
			memo[i][j] = make([]int, len(t)+1)
	...
	solve = func(i, j int) int {
		...
		if memo[i][j][k] == -1 {
			if i == len(s1) && j == len(s2) && k == len(t) {
				...
				if i < len(s1) && s1[i] == t[k] && solve(i+1, j) == 1 {
					memo[i][j][k] = 1
				}

				if j < len(s2) && s2[j] == t[k] && solve(i, j+1) == 1 {
					memo[i][j][k] = 1
				}
			}
		}
