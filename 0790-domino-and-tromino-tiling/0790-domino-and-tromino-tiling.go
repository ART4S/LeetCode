func numTilings(n int) int {
	...
	for i := 0; i < n; i++ {
		for p := 0; p < len(profiles); p++ {
			for _, nextp := range profiles[p] {
				ways[i+1][nextp] = (ways[i+1][nextp] + ways[i][p]) % mod
			}
		}
	}
...
func numTilings_memo(n int) int {
	const mod = 1e9 + 7
