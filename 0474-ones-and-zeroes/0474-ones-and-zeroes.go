func findMaxForm(strs []string, m int, n int) int {
	...
	for i := 1; i <= len(nums); i++ {
		for o := 0; o <= n; o++ {
			for z := 0; z <= m; z++ {
				if nums[i-1][1] > o || nums[i-1][0] > z {
					dp[i][o][z] = dp[i-1][o][z]
				} else {
					dp[i][o][z] = Max(dp[i-1][o][z], 1+dp[i-1][o-nums[i-1][1]][z-nums[i-1][0]])
				}
			}
		}
	}
...
func findMaxForm_memo(strs []string, m int, n int) int {
	nums := make([][]int, len(strs))
	...
	solve = func(i, o, z int) int {
		if memo[i][o][z] == -1 {
			if i == len(nums) {
				memo[i][o][z] = 0
			} else if nums[i][1] > o || nums[i][0] > z {
				memo[i][o][z] = solve(i+1, o, z)
			} else {
				memo[i][o][z] = Max(solve(i+1, o, z), 1+solve(i+1, o-nums[i][1], z-nums[i][0]))
			}
		}
