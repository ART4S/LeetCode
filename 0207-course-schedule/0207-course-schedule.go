func canFinish(numCourses int, prerequisites [][]int) bool {
	cources := make([][]int, numCourses)

	for i := range cources {
		cources[i] = make([]int, 0)
	}

	for i := range prerequisites {
		cources[prerequisites[i][0]] = append(cources[prerequisites[i][0]], prerequisites[i][1])
	}

	visited := make(map[int]bool, numCourses)
	finished := make(map[int]bool, numCourses)

	var isFinished func(int) bool

	isFinished = func(course int) bool {
		if visited[course] {
		...
		for _, prevCourse := range cources[course] {
			if !isFinished(prevCourse) {
				return false
			}
		}
