...
func mctFromLeafValues_memo(arr []int) int {
	mx := make([][]int, len(arr))
	...
	solve = func(l, r int) int {
		...
		if memo[l][r] == -1 {
			...
			for m := l; m < r; m++ {
				res = Min(res, solve(l, m)+solve(m+1, r)+mx[m+1][r]*mx[l][m])
			}
...
// 		// fmt.Printf("start f(%v..%v):\n", arr[l], arr[r])

// 		for m := l; m <= r; m++ {
// 			if m == l {
// 				// fmt.Printf("f(%v) * %v + f(%v..%v)\n", arr[l], mx[l+1][r], arr[l+1], arr[r])
// 				res = Min(res, arr[l]*mx[l+1][r]+solve(l+1, r))
// 			} else if m == r {
// 				// fmt.Printf("f(%v..%v) + f(%v) * %v\n", arr[l], arr[r-1], arr[r], mx[l][r-1])
// 				res = Min(res, arr[r]*mx[l][r-1]+solve(l, r-1))
// 			} else {
// 				// fmt.Printf("f(%v..%v) * %v + f(%v..%v) * %v\n", arr[l], arr[m], mx[m+1][r], arr[m+1], arr[r], mx[l][m])
// 				res = Min(res, mx[m+1][r]*solve(l, m)+mx[l][m]*solve(m+1, r))
// 			}
// 			// if m == l {
// 			// 	res = Min(res, arr[m]*mx[m+1][r]+solve(m+1, r))
// 			// } else if m == r {
// 			// 	res = Min(res, arr[m]*mx[l][m-1]+solve(l, m-1))
// 			// } else {
// 			// 	res = Min(res, mx[m+1][r]*solve(l, m)+mx[l][m]*solve(m+1, r))
// 			// }
// 		}
