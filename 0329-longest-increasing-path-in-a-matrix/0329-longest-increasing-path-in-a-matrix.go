func longestIncreasingPath(matrix [][]int) int {
	...
	for !done {
		...
		for i := 0; i < n; i++ {
			for j := 0; j < m; j++ {
				if i-1 >= 0 && matrix[i-1][j] < matrix[i][j] && dp[i][j] <= dp[i-1][j] {
					dp[i][j] = 1 + dp[i-1][j]
					done = false
				}

				if i+1 < n && matrix[i+1][j] < matrix[i][j] && dp[i][j] <= dp[i+1][j] {
					dp[i][j] = 1 + dp[i+1][j]
					done = false
				}

				if j-1 >= 0 && matrix[i][j-1] < matrix[i][j] && dp[i][j] <= dp[i][j-1] {
					dp[i][j] = 1 + dp[i][j-1]
					done = false
				}

				if j+1 < m && matrix[i][j+1] < matrix[i][j] && dp[i][j] <= dp[i][j+1] {
					dp[i][j] = 1 + dp[i][j+1]
					done = false
				}
			}
